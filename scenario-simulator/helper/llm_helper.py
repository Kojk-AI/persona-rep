...
class LLMHelper:
    ...
    def _query_openai(self, conversation_history: list, model_name: str, llm_params: dict, require_json_output: bool) -> str:
        """
    ...
    def _query_ollama(self, conversation_history: list, model_name: str, llm_params: dict, require_json_output: bool) -> str:
        """
    ...
    def call_llm_one_off(self, prompt, model_name='llama2', llm_params: dict = {}, llm_response_csv_fieldname='response', log_file=None, system_prompt=None, require_json_output: bool = False, **kwargs) -> str:
        """
        ...
        if model_name.startswith('gpt'):
            ...
            if require_json_output: data["response_format"] = {"type": "json_object"}
            ...
            if require_json_output: data['format'] = 'json'
            # send request
            response = requests.post(url, json=data)
    ...
    def call_llm(self, prompt: Union[str, List[str]], model_name: Union[str, List[str]], conversation_id: str = None, llm_params: dict = {}, system_prompt: str = None, require_json_output: bool = False, log_file: str = None, **kwargs) -> Dict[str, str]:
        """
        ...
        Examples:
            >>> llm_helper = LLMHelper()
            >>> response = llm_helper.call_llm(prompt='Who are you?', model_name='gpt-3.5-turbo', log_file='log.csv')
            >>> print(response)
            
            >>> prompts = ['Tell me a joke', 'What is AI?']
            >>> models = ['gpt-3.5-turbo', 'llama2']
            >>> response = llm_helper.call_llm(prompt=prompts, model_name=models, log_file='log.csv')
            >>> print(response)
        """
        ...
        for single_prompt in prompt:
            for single_model_name in model_name:
                ...
                if single_model_name.startswith('gpt'):
                    answer = self._query_openai(conversation_history, single_model_name, llm_params, require_json_output)
                    ...
                    answer = self._query_ollama(conversation_history, single_model_name, llm_params, require_json_output)
                ...
                # responses[f'{single_model_name}_{conversation_id}'] = answer
